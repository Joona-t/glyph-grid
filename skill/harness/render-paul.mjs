/* render-paul.mjs — render the paul-atreides-dune piece into a GIF.
 * Copy of render-gif.mjs with PIECE_DIR + OUT_GIF pointed at Paul. */
...
const PIECE_DIR = resolve(__dirname, '..', '..', 'pieces', 'paul-atreides-dune');
const OUT_GIF  = process.argv[2] || resolve(homedir(), 'Downloads', 'paul-atreides__dune.gif');
...
const ctx = await browser.newContext({ viewport: { width: 880, height: 880 }, deviceScaleFactor: 1, acceptDownloads: true });
const page = await ctx.newPage();
page.on('pageerror', (e) => console.warn('[err]', e.message.slice(0, 200)));
await page.goto(url + (process.env.GG_QS || ''), { waitUntil: 'load' });
...
// Also save samples for quick inspection
for (const i of [0, Math.floor(frames.length/2), frames.length-1]) {
  writeFileSync('/tmp/paul-sample-f' + String(i).padStart(2, '0') + '.png', frames[i]);
}
